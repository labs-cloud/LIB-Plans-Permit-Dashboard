...
export function Dashboard({ initial, initialError }: Props) {
  ...
  const filteredKpis: KpiStripData = useMemo(
    () => (coord === 'all' && phase === 'all' && !search ? payload.kpis : computeKpis(filtered)),
    [coord, phase, search, filtered, payload.kpis],
  );
  const filteredSticking: StickingItem[] = useMemo(
    () => (coord === 'all' && phase === 'all' && !search ? payload.sticking : computeSticking(filtered)),
    [coord, phase, search, filtered, payload.sticking],
  );
  const filteredPermits: PermitsPanelData = useMemo(
    () => (coord === 'all' && phase === 'all' && !search ? payload.permits : computePermitsPanel(filtered)),
    [coord, phase, search, filtered, payload.permits],
  );
